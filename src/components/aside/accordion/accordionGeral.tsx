...
export default function AccordionGeral() {
  ...
  useEffect(() => {
    async function fetchFollowers() {
      try {
        const response = await api.get(`users/HeitorSantos420/followers`);
  ...
  return (
    ...
    >
      <AccordionItem value="item-1">
        <AccordionTrigger className="flex w-full items-center gap-2 px-2 py-1 uppercase text-client-TextSecondary opacity-80 hover:bg-[#e8edf928] hover:opacity-100">
          <Play
            size={10}
        ...
        <AccordionContent className="max-h-[72vh] overflow-auto">
